...
/** @defgroup USBD_HID_Private_Variables
  * @{
  */
uint8_t gHidUserOutgoingStatusBufferToHost[HID_REPORT_LENGTH_STATUS+1];
uint8_t gHidUserOutgoingReportBufferToHost[USB_MAXIMUM_DEVICE_TO_HOST_PACKET_SIZE];
uint8_t gHidUserIncomingReportBufferFromHost[USB_MAXIMUM_HOST_TO_DEVICE_PACKET_SIZE];
USBD_HID_HandleTypeDef gHidHandleTypeDef;
...
static uint8_t  USBD_HID_Init (USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  pdev->pClassData = &gHidHandleTypeDef; // USBD_malloc(sizeof (USBD_HID_HandleTypeDef));
  ...
  hhid->incomingReportBufferFromHost = &gHidUserIncomingReportBufferFromHost[0];
  hhid->incomingReportBufferFromHostReceivedSize = 0;
  hhid->incomingReportBufferFromHostMaximumSize = sizeof(gHidUserIncomingReportBufferFromHost);

  hhid->outgoingReportBufferToHost = &gHidUserOutgoingReportBufferToHost[0];
  hhid->outgoingReportBufferToHostSendSize = 0;
  hhid->outgoingReportBufferToHostMaximumSize = sizeof(gHidUserOutgoingReportBufferToHost);

  hhid->outgoingStatusBufferToHost = &gHidUserOutgoingStatusBufferToHost[0];
  hhid->outgoingStatusBufferToHostSendSize = HID_REPORT_LENGTH_STATUS + 1;
  hhid->outgoingStatusBufferToHostMaximumSize = sizeof(gHidUserOutgoingStatusBufferToHost);
                                ...
                                USBD_SetupReqTypedef *req)
{
  ...
  case USB_REQ_TYPE_CLASS :
    ...
    case HID_REQ_SET_REPORT:
      hhid->incomingReportBufferFromHostReceivedSize = req->wLength;
      hhid->reportState = HID_REPORT_INCOMING;
      USBD_CtlPrepareRx (pdev, hhid->incomingReportBufferFromHost, (uint16_t)(req->wLength));
      break;
...
static uint8_t  USBD_HID_DataOut (USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  USBD_LL_PrepareReceive(pdev, HID_EPOUT_ADDR, hhid->outgoingStatusBufferToHost,
                         USB_MAXIMUM_DEVICE_TO_HOST_BUFFER_SIZE);

  return USBD_OK;
}

/**
  * @brief  USBD_HID_EP0_TxSent
  *         Called when status is finished.
  * @param  pdev: device instance
  * @retval status
  */
uint8_t USBD_HID_EP0_TxSent(USBD_HandleTypeDef *pdev)
{
//  USBD_HID_HandleTypeDef     *hhid = (USBD_HID_HandleTypeDef*)pdev->pClassData;
